...
houses = {    # Dictionnaire permettant de savoir la maison qui joue, son score et sa couleur
    "Ravenclaw": {
        "Playing": False,
        "Color": "blue",
        "Points": 0
...
def text_format(message, textFont, textSize, textColor):
    ...
    Entrée: le texte à afficher, la police d'écriture, la taille de la police et la couleur du texte
...
def main_menu(first_menu):
    ...
    Entrée: un booléen qui indique si l'on se situe au premier menu ou au second
    """
    ...
    for event in pg.event.get():    # On liste tous les évenements qui se sont déroulés entre les deux frames
        ...
        if event.type == pg.KEYDOWN:    # L'utilisateur a appuyé sur le clavier
            if event.key == pg.K_UP:    # L'utilisateur a appuyé sur la flèche du haut
                if first_menu:
                    selected = "Start"  # On change donc le texte sélectionné
                elif selected == "Back":
                ...
                if first_menu:
                    if selected == "Start":
                        selected = "10 bets"
                        is_this_main_menu = False   # On change de menu
                    elif selected == "Quit":
                        pg.quit()
                        quit()
                else:
    ...
    # On affiche un titre au menu et on met en plus gros et en blanc ce qui est sélectionné par l'utilisateur,
...
def match_anouncement():
    """
    Annoncement du match qui va opposer deux équipes, c'est en fait une petite animation qui va permettre
    de décider les deux équipes joueuses
    """

    for event in pg.event.get():    # Si l'utilisateur veut fermer la fenêtre on quitte l'application
    ...
    # On détermine au hasard soit un match Gryffondor - Serpentard soit Poufsouffle - Serdaigle
    if randint(0, 1) == 0:
    ...
    screen.blit(title, (screen_width/2 - (title_rect[2]/2), 50))
    pg.display.update()     # On actualise l'écran pour y afficher notre interface
...
def anim_scores(round, prev1, prev2, snitch):
    """
    Anime l'augmentation des scores en ajoutant au fur et à mesure d'un "temps" les points gagnés
    toutes les "5 minutes" puis affiche le vainqueur du match
    
    Entrée: la manche à cet instant ce qui permet d'extrapoler le temps passé (1 manche = 5 min)
            le score de l'équipe 1 et 2 précédant le tirage au sort des points gagnés à cette manche 
            un booléen indiquant si le vif d'or à été capturé ce qui arrêterai la partie
    """
    ...
    Time = 0 # variable qui nous permet de stopper le temps quand le vif d'or est capturé
    for i in range(1, 6):   # On répète 5 fois la boucle, une fois par "minute", 5 fois pour 5 "minutes"
        ...
        screen.fill(black)
        # On affiche un rectangle de 400 par 300 pixels à 200 pixels de la droite de l'écran et 125 du haut
        ...
        if snitch:
            ...
            if round*5 + 1 != Time: # On "arrête" le temps
                Time = round*5 + 1
            # On se prépare à afficher le temps fictif en plus du nombre de points de chaque équipe
            ...
            # Ici le temps n'est pas statique il dépends du nombre d'itération de la boucle
        ...
        # Si on est à la manche 7 et à la 5ème itération de la boucle le temps est écoulé
        if round == 7 and i == 5:
...
def play_5min():
    ...
    for equipe in (("Hufflepuff", "Ravenclaw") if houses["Ravenclaw"]["Playing"] else ("Gryffindor", "Slytherin")):
        ...
        if randint(1, 50) == 25:    # Une chance sur 50 d'avoir 150 points en plus et de terminer la partie
...
def bet():
    ...
    Les paris vont dans la variable characters au niveau du pari de chaque personnage
    """

    global characters
    playing = ("Gryffindor", "Slytherin") if houses["Gryffindor"]["Playing"] else ("Hufflepuff", "Ravenclaw")
    for i in range(len(characters)):
        # Si un personnage fait partie d'une des équipes jouant à cette partie et qu'il à plus d'un point alors il peut parier,
        # on lui retire donc un point et il parie un nombre aléatoire multiple de 10 entre la valeur max et la minimum
...
def distribute_points():
    """
    Distribue 5 points aux joueurs de l'équipe gagnante et 30 à la personne la plus proche du score
    """
    ...
    best_bet = [999, 0]
    for i in range(len(characters)):    # On itère dans toute la liste de personnages pour leur rajouter des points ou non
        if characters[i]["House"] == winner:
            characters[i]["Points"] += 5
            # Si une personne à plus ou moins le score le plus proche on sauvegarde son score si une autre personne fait mieux
...
def anim_games(nb_game):
    """
    Affichage de toutes les parties éffectuées (20) au fur et à mesures qu'elles sont jouées,
    on y affiche le score coloré en fonction de l'équipe et le numéro du match
    ...
    # Au bout de 5 parties on rajoute les rectangles sur la ligne du dessous et ainsi de suite jusqu'à 20 rectangles
...
def leaderboard():
    """
    Affiche le tableau des scores classés dans l'ordre décroissant avec le nom du personnage,
    sa maison (couleur du personnage), son score et son classement
    """

    global selected, is_this_main_menu, characters

    # On trie les perssonnages de characters en fonction de leur score dans l'ordre décroissant
    characters = sorted(characters, key=lambda x: x["Points"], reverse=True)

    screen.fill(black)
    for i in range(14, -1, -1): # On commence par le bas du tableau puis on remonte
    ...
    for i in range(len(characters)):
        print(f"{i+1} - {characters[i]['Name']} with {characters[i]['Points']} points")

    # Réinitialise toutes les variables pour se retrouver au menu principal avec comme sélection "Start"
    selected = "Start"
    is_this_main_menu = True
...
def winner():
    """
    Affiche le vainqueur des paris, celui qui à gagné le plus de points et affiches quelques unes de ses caractéristiques
    """
...
def game(single):
    """
    On va simuler une ou 20 parties en plus de gérer les paris de chaques personnages

    Entrée: une booléen indiquant si il s'agit d'une partie simple ou de 20 parties
    """

    global houses, vifdor
    vifdor = False
    
    # Si on ne simule qu'une partie, on lance l'animation de l'annoncement du match + celle des points gagnés par chaque équipe par rapport à la simu
    if single:  
        ...
        for manche in range(8):     # On joue 8 manches de "5 min" = "40 min"
            ...
            anim_scores(manche, prev[0], prev[1], vifdor)   # On anime le gain de score pendant ces "5 minutes"
    ...
    # On va simuler 20 parties
        ...
        for game in range(20):  # On répète pour les 20 partie
            ...
            vifdor = False
            # Une fois sur deux ce sera le match Gryffondor contre Serpentard et l'autre fois Poufssoufle contre Serdaigle 
            if game % 2 == 0:
                houses["Gryffindor"]["Playing"] = True
                houses["Slytherin"]["Playing"] = True
            ...
            for _ in range(8):  # On simule une partie de "40 minutes"
                ...
                if vifdor:
                    break
            distribute_points()     # On donne les points une fois la partie terminée
...
def running():
    ...
    while 1:
        ...
        clock.tick(FPS)     # On définit la vitesse de rafraichissement à 30 FPS
